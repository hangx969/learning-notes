...
- `a.com` 的 Cookie 不会自动发送给 `b.com`；两个不同域名的应用不能直接共享同一个 Cookie。
- 即使多个应用使用共享会话存储，也需要协调会话格式、用户标识和信任边界；跨域 SSO 通常由统一身份提供方处理。
- SSO 通常通过统一身份提供方和浏览器重定向实现：用户在认证中心登录后，访问另一个系统时会再次跳转到认证中心，由认证中心确认登录状态并向该系统颁发凭据。不同域名的应用仍各自维护 Cookie 或会话。
- JWT 可以签名，也可以加密；常见的签名 JWT 对编码后的 header 和 payload 签名，接收方验证签名及必要的声明。签名不等于加密，payload 不应被视为保密。跨系统用户身份还需统一的标识与信任关系。
  ...
  - SSO-统一认证中心 CAS
    ...
    **首次登陆过程:**
    ...
    - 客户端浏览器访问a.com/pageA,这个页面需要登录才能访问;系统A服务端返回302,将请求重定向到sso.com(www.sso.com/login?redirect=www.a.com/pageA),sso让客户端去登陆，登陆成功后:
      ...
      - SSO 创建一次性 ticket（可视为随机字符串）。

      - 认证中心通过自己的 Cookie 维持登录会话；ticket 随后通过重定向 URL 传给系统 A。
...
- 密码凭据许可：资源所有者（即网易邮箱用户）直接向客户端 App 提供用户名和密码。这是 RFC 6749 描述的历史授权类型；RFC 9700 已明确要求不再使用。
...
- App 需要先在授权服务器注册并获得 client_id；浏览器端的公开客户端不能安全保存 client_secret。
- App 提供“使用网易账号登录”入口，浏览器重定向到授权服务器，携带公开的 client_id 等参数；不能在浏览器请求中传递 client_secret。
...
- OIDC 的 ID Token 通常是签名的 JWT；用户身份等声明位于 payload 中，客户端需验证签名以及 issuer、audience、有效期等声明。
