...
nmcli connection modify ens33 ipv4.addresses 192.168.40.180/24
nmcli connection modify ens33 ipv4.gateway 192.168.40.2
nmcli connection modify ens33 ipv4.dns 192.168.40.2
...
在工作节点（rn1）上执行返回的 join 命令：

```sh
# 示例（替换为实际输出的命令）
kubeadm join 192.168.40.180:6443 \
  --token <token> \
