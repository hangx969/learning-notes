...
nmcli con mod "ens160" ipv4.dns "192.168.164.2"
...
删除 swap 分区后必须重新配置 grub、dracut 和 initramfs，否则重启会卡住或报 "找不到 swap" 错误：
...
# 2. 用 sed 自动删除 swap 相关的两个参数（无需手动 vim 编辑）
...
# 3. 验证修改结果（确认 swap 相关参数已消失）
grep GRUB_CMDLINE_LINUX /etc/default/grub

# 4.【关键】清除 dracut 的 resume 模块配置
#    dracut 有独立的 resume 配置，仅清理 GRUB 参数不够！
...
# 5. 重新生成 initramfs（此步不可跳过，必须在上一步清除 resume 配置之后执行）
dracut -f

# 6. 重新生成 grub 配置
[ -d /sys/firmware/efi ] && echo UEFI || echo BIOS   # 判断引导方式
...
> [!warning] 常见错误：重启后卡住或报"找不到 /dev/rl_rm1/swap"
> 原因是以下**三处**任一未清理（前两处仅清理 GRUB 参数是不够的）：
> 1. `GRUB_CMDLINE_LINUX` 里还有 `resume=` 或 `rd.lvm.lv=.../swap`（grub 步骤未做）
> 2. **dracut 的 resume 模块仍然启用**（最容易遗漏！即使 GRUB 参数已清理，`dracut -f` 生成的 initramfs 仍会包含 resume 模块，启动时尝试激活已删除的 swap LV 导致卡住）
> 3. initramfs 未重新生成（`dracut -f` 步骤未做，或在清除 resume 配置之前就执行了）
>
> 注意 Rocky Linux 10 安装时 VG 名会带主机名（如主机名 rm1 → VG 名为 `rl_rm1`），文档示例中的 `rl_rm1` 请替换为实际 VG 名（通过 `vgs` 查看）。
...
从官网（[https://docs.tigera.io/calico/latest](https://docs.tigera.io/calico/latest/getting-started/kubernetes/self-managed-onprem/onpremises)）获取最新版 Calico的manifest：
