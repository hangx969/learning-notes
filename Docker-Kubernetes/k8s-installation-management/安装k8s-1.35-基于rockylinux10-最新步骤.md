...
nmcli con mod "ens160" ipv4.dns "223.5.5.5 8.8.8.8 192.168.164.2"
...
删除 swap 分区后必须重新配置 grub 和 initramfs，否则重启会报 "找不到 swap" 错误：
...
# 4. 重新生成 initramfs（清除 initramfs 中对 swap LV 的引用，此步不可跳过）
dracut -f
...
> [!warning] 常见错误：重启后报"找不到 /dev/rl_rm1/swap"
> 原因是以下两处任一未清理：
> - `GRUB_CMDLINE_LINUX` 里还有 `resume=` 或 `rd.lvm.lv=.../swap`（grub 步骤未做）
> - initramfs 里还有 swap LV 引用（`dracut -f` 步骤未做）
>
