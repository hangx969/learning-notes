...
# 以 ens33 为例，修改为静态 IP（各节点 IP 不同，按实际修改）
nmcli connection modify ens33 ipv4.method manual connection.autoconnect yes
nmcli connection modify ens33 ipv4.addresses 192.168.164.100/24  # 各节点 IP 不同，按实际修改
nmcli connection modify ens33 ipv4.gateway 192.168.164.2
nmcli connection modify ens33 ipv4.dns 192.168.164.2
nmcli connection down ens33 && nmcli connection up ens33
```

> [!tip]
> NetworkManager 会把修改写入 `/etc/NetworkManager/system-connections/ens33.nmconnection`；手动编辑该文件也可，但改完需执行 `nmcli connection reload`
...
lvextend -L +<SIZE>G /dev/rl/root   # 扩容 root LV（替换 SIZE 为实际空闲大小）
...
> [!info] 关于 containerd 2.x
> containerd 2.0 是重大版本升级，引入了全新的配置格式（config version = 3），CRI 插件命名空间从 `io.containerd.grpc.v1.cri` 拆分为 `io.containerd.cri.v1.images` 和 `io.containerd.cri.v1.runtime`。镜像加速改为目录方式配置。

### 安装 containerd

**方式一：通过 Docker CE 仓库安装（推荐）**

```sh
# 添加 Docker CE RHEL 源（适用于 RHEL 10 / Rocky Linux 10）
...
**方式二：下载二进制安装（Docker CE 源暂不支持时使用）**

```sh
# 下载最新版 containerd（访问 github.com/containerd/containerd/releases 获取最新版本号）
CONTAINERD_VERSION="2.1.0"  # 替换为最新版本
wget https://github.com/containerd/containerd/releases/download/v${CONTAINERD_VERSION}/containerd-${CONTAINERD_VERSION}-linux-amd64.tar.gz
tar Cxzvf /usr/local containerd-${CONTAINERD_VERSION}-linux-amd64.tar.gz

# 下载 systemd service 文件
wget -O /etc/systemd/system/containerd.service \
...
# 2. 替换 sandbox (pause) 镜像为国内镜像（k8s 1.35 对应 pause 3.10）
sed -i 's#registry.k8s.io/pause:[0-9.]*#registry.cn-hangzhou.aliyuncs.com/google_containers/pause:3.10#g' /etc/containerd/config.toml

# 3. 配置 registry 镜像目录（containerd 2.x 推荐方式）
sed -i 's#config_path = ""#config_path = "/etc/containerd/certs.d"#g' /etc/containerd/config.toml
```

也可直接使用如下最小化配置（containerd 2.x，config version = 3）：
...
[plugins."io.containerd.cri.v1.images"]
  ...
  # 使用目录方式配置镜像加速（containerd 2.x 推荐）
...
> [!warning] containerd 2.x 与 1.x 配置差异
> | 配置项 | containerd 1.x | containerd 2.x |
> |--------|---------------|----------------|
> | config version | `version = 2` | `version = 3` |
> | CRI 插件前缀 | `io.containerd.grpc.v1.cri` | `io.containerd.cri.v1.images` / `io.containerd.cri.v1.runtime` |
> | 镜像加速配置 | 内联 mirrors 块 | 目录方式（`config_path`） |
>
> 若安装的是 containerd 1.7.x（LTS），则仍使用旧格式（version = 2），可参考 [[安装k8s-1.33-基于rockylinux-最新步骤]] 中的配置。

### 配置镜像加速（containerd 2.x 目录方式）
...
# 配置 registry.k8s.io 镜像加速（拉取 k8s 核心组件镜像）
...
kubernetesVersion: 1.35.0  # 指定 K8s 版本
networking:
...
> - **证书有效期**：新增 `certificateValidityPeriod`（默认 1 年）和 `caCertificateValidityPeriod`（默认 10 年）字段

```sh
# 【可选】提前拉取所需镜像（避免初始化时因网络问题失败）
...
# 将 ens33 替换为实际网卡名（通过 ip addr 查看）
...
ctr -n=k8s.io images export k8s-1.35.0-images.tar.gz ${IMAGE_LIST}

# 在内网机器导入镜像
ctr -n=k8s.io images import k8s-1.35.0-images.tar.gz
