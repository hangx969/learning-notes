...
- 常用output
  ...
  - graphite:将事件数据发送给 graphite(一个流行的开源工具,存储和绘制指标,[Graphite文档](http://graphite.readthedocs.io/en/latest/))。
...
> [!warning] k8s 1.20+版本nfs provisioner报错
> k8s1.20+版本通过nfs provisioner动态生成pv会报错: Unexpected error getting claim reference to claim "default/test-claim1": selfLink was empty, can't make reference,报错原因是1.20版本启用了selfLink,解决方法如下;
>
