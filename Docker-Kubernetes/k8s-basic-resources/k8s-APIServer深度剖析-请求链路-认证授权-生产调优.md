...
**MutatingAdmissionWebhook（变更）→ 先执行**
**ValidatingAdmissionWebhook（校验）→ 后执行**
...
1. **ResourceVersion**：类似 etcd 的 revision，客户端 Watch 时携带上次的 RV，只接收之后的增量事件
2. **Watchcache**：API Server 内存中维护了所有资源的缓存，Watch 先从缓存服务，减少 etcd 压力
3. **Bookmark 事件**：周期性发送，更新客户端的 RV，避免过期重新全量 List
...
1. **请求链路**：认证 → 授权 → 变更准入 → 校验 → 持久化 → Watch 通知，每个环节都可能是问题根源
2. **Watch 机制**：基于 ResourceVersion 的增量推送，Informer 是正确的使用姿势
3. **准入 Webhook**：强大但危险，`failurePolicy: Fail` 配置不当会引发集群级联故障
4. **高可用部署**：3 副本 + 负载均衡是生产标配
5. **监控告警**：重点关注请求延迟、错误率和并发请求数
6. **审计日志**：生产环境必须开启，安全合规的基础
