...
> [!summary] ESS核心能力
> **扩容时:**
> - 自动创建指定数量、指定类型的实例([[Aliyun/计算/ECS|ECS]]/ECI实例)
> - 如果伸缩组关联了[[Aliyun/网络/负载均衡SLB|负载均衡]],自动为创建的ECS关联负载均衡
> - 如果伸缩组关联了[[Aliyun/数据库/关系型数据库RDS|RDS]],自动将创建的ECS实例IP加到RDS访问白名单中。
>
> **缩容时:**
...
实例来源可以选择==实例启动模板==和==伸缩配置==两种。

## 启动模板

- 用某个自定义[[Aliyun/计算/云盘-快照-镜像|镜像]]启动实例。
...
定义对哪些[[Aliyun/计算/ECS|ECS]]实例做扩缩容,定义实例最大最小数量,关联的[[Aliyun/网络/负载均衡SLB|负载均衡]]实例、关联的[[Aliyun/数据库/关系型数据库RDS|RDS]]实例等。
...
> [!important] 手动添加 vs 自动创建的实例
> 手动添加进伸缩组的实例,缩容时==不会被删除==,只会被移除出伸缩组;而自动扩容出来的实例,缩容时==会被释放掉==。

> [!note] 包年包月实例在弹性伸缩中的角色
...
e.g. 持续监测CPU使用率,自动伸缩,使得CPU使用率一直保持在==50%==左右
...
弹性伸缩提供健康检查功能,自动监控伸缩组内[[Aliyun/计算/ECS|ECS]]实例的健康状态。监测到某台实例不健康(即未处于运行中状态),==自动释放不健康实例并创建新的实例==。
...
添加了生命周期挂钩之后:
- 扩容出新实例的时候,会出现==Pending:Wait==状态,在这期间进行预装软件或其他自定义操作。
- 缩容的时候,出现==Removing:Wait==状态。

> [!example] 适合配置生命周期挂钩的场景
...
- 部署在伸缩组内的ECS实例,需要时==无状态==的,不要存储持久化数据。应用数据、日志存在单独的地方
- 如果伸缩组关联的[[Aliyun/数据库/关系型数据库RDS|RDS]]实例、ALB/CLB服务器组被删除,那么伸缩组自动解除与该资源的关联;但是伸缩活动不会因为解除关联而停止。
- 保证ECS级别的事务完整性:即只进行ECS实例级回滚,而不是伸缩活动回滚。比如,创建了20台ECS实例,只有19台成功加入负载均衡,不成功的一台实例会进行自动释放,而不是所有20台都释放掉。
- 伸缩活动==无法中断==,比如扩容5台,在扩容到第三台的时候无法强行终止。
...
> - 在冷却时间内,只有==云监控报警任务==类型的伸缩活动请求会被拒绝,其他类型的触发任务(手动、定时等)可以绕过冷却时间立即执行伸缩活动。
