...
![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501224926431.gif)

一旦用户在前端发出提问，这个消息就会通过 WebSocket 通道发送到后端。后端接收到消息后会去调用知识库去做一次混合检索，找出相关的文本内容后，再拼接上用户的历史上下文，构建一个完整的 Prompt。

![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501224929024.png)
...
![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501224932453.png)
...
![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501224935924.png)

具体来说，HTTP 是一种无状态、单向的请求-响应协议，它的工作机制决定了只能由客户端发起请求，服务端只是被动响应。哪怕我们用长轮询等手段模拟实时性，本质上还是客户端不断地问“有没有新的消息”，服务器无法主动发“有了”。

![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501224938638.png)

WebSocket 是一种有状态、全双工的协议，一旦连接建立，前后端就可以随时互相发送消息。在RAG-Agent中，前端发起 WebSocket 请求建立连接后，后端就会一直监听前端的请求并保持连接。用户一旦发送请求，RAG-Agent就会实时将 DeepSeek 返回的是流式数据通过 WebSocket 传回给前端。

![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501224941613.png)
...
![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501224944347.png)
...
![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501224947232.png)

后端在收到这个 WebSocket 连接请求后，并不会立即建立连接，而是先进行身份校验。具体的做法是，每次处理消息之前，都会从 WebSocketSession 中解析出前端发送过来的 Token，然后去校验这个 Token 的有效性。

![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501224950616.png)
...
![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501224953350.png)
...
![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501224956613.png)
...
![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501224959319.png)

也就是说，只要有会话 ID，我们就可以从 Redis 中获取到完整的聊天记录（一个JSON数组）。

## 8.大语言模型的输入长度（Token窗口）是有限的。如果对话越来越长，你是如何处理这个上下文窗口，避免它超出限制的？

RAG-Agent目前采用的策略是滑动窗口。也就是每次只保留最近的几轮对话历史，大概 20 条消息。
...
## 9.RAG在回答专业问题时，不仅仅是在“创作”，而是在“引用”一些知识。RAG-Agent是怎么集成本地知识库的？

第一步，接收到用户的提问后，我们会先进行一次混合检索。我们会根据用户输入的内容，在 Elasticsearch 里查出前 N 个最相关的知识片段。

![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501225002169.png)
...
![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501225004838.png)
...
![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501225008120.png)

在动态构建提示词的时候， <u>我们首先会构建一个 system 指令，然后将检索到的知识片段包裹在引用开始/结束符中，并且要求模型遵守我们自定义的规则。</u>

![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501225010896.png)
...
![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501225014347.png)
