RAG-Agent是一个基于 RAG 的 AI 知识库智能问答平台，通过结合大语言模型（如 DeepSeek）和向量化技术，为用户提供高效、精准的问答服务。

用户可以通过上传文档构建专属的私有知识库，并实时与RAG-Agent进行交互提问。RAG-Agent支持多轮对话、语义检索和动态知识库更新，适用于企业内部知识管理、教育、客服等场景。

那基于RAG-Agent的需求分析，我们梳理了下列需求（思维导图一目了然）：

![](https://raw.githubusercontent.com/hangx969/upload-images-md/main/20260501223927767.jpeg)

## 一、业务架构设计

基于RAG-Agent的需求梳理，我们设计了这样一个三层业务架构图：
...
RAG-Agent的文档处理与向量化是一个完整的自动化流程，涉及用户、文件服务、向量化服务、Elasticsearch 和 MySQL 等多个组件的协同工作。
...
RAG-Agent实现了一套完整的组织权限管理体系，从管理员创建组织标签到用户文件访问控制形成了完整的权限管理闭环。
...
RAG-Agent采用了分层架构设计，从上到下分为前端展示层、API 与安全层、业务逻辑层、数据访问层和数据存储层，同时集成了外部依赖服务，形成了完整的企业级应用架构。
...
RAG-Agent还集成了多个外部服务。DeepSeek API 用于智能对话和内容生成。Embedding API 负责文本向量化，将自然语言转换为数学向量，这是实现语义检索的关键。
...
数据一致性是大多数系统面临的核心挑战，RAG-Agent通过多层次的保障机制来确保数据在 MySQL、Elasticsearch 和 MinIO 三个存储系统中的一致性。我们为每个文件建立了完整的生命周期跟踪机制。
...
RAG-Agent采用了多层防护策略，JWT 身份认证作为第一道防线，提供无状态的用户身份验证机制。同时，令牌的过期机制和刷新策略，也能够最大程度确保用户身份的安全与登录体验。
...
就目前来说，RAG-Agent已经通过整合 RAG 技术和多租户权限控制，为企业提供了一个高效、安全、基于 AI 的知识库问答平
