...
- **14 个图像生成后端**（每个后端单独配 API Key，切换时只改 `IMAGE_BACKEND`）：
  ...
  | 分类 | 后端 | 环境变量 | 默认模型 |
  |------|------|----------|----------|
  | 核心 | gemini | `GEMINI_API_KEY` | gemini-3.1-flash-image-preview |
  | 核心 | openai | `OPENAI_API_KEY` | gpt-image-1 |
  | 核心 | qwen（通义） | `QWEN_API_KEY` | qwen-image-2.0-pro |
  | 核心 | zhipu（智谱） | `ZHIPU_API_KEY` | glm-image |
  | 核心 | volcengine（火山引擎） | `VOLCENGINE_API_KEY` | doubao-seedream-4-5-251128 |
  | 扩展 | stability | `STABILITY_API_KEY` | stable-image-core |
  | 扩展 | bfl（Black Forest Labs） | `BFL_API_KEY` | flux-pro-1.1-ultra |
  | 扩展 | ideogram | `IDEOGRAM_API_KEY` | ideogram-v3 |
  | 实验 | minimax | `MINIMAX_API_KEY` | image-01 |
  | 实验 | siliconflow（硅基流动） | `SILICONFLOW_API_KEY` | Qwen/Qwen-Image |
  | 实验 | fal | `FAL_KEY` | fal-ai/imagen3/fast |
  | 实验 | replicate | `REPLICATE_API_TOKEN` | flux-1.1-pro |
