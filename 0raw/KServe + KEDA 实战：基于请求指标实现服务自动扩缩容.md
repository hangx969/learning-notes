...
source: "https://mp.weixin.qq.com/s/w0wBObGfWXUye75e14rveg?scene=1&click_id=76637624"
author:
  - "[[意琦行]]"
published:
created: 2026-09-06
description: "模型服务运行后，固定副本数很难同时兼顾突发请求和资源利用率。请求增加时需要扩容，流量回落后又希望及时缩容。本文通过一个简单 Demo，为 KServe 模型服务接入 KEDA，根据 vLLM 的请求指标自动调整副本数，并验证服务从 `1 -\x26gt; 2 -\x26gt; 1` 的扩缩容过程。"
tags:
  - "clippings"
---
意琦行 探索云原生 *2026年8月26日 08:00*

![KServe 集成 KEDA：基于 vLLM 指标自动扩缩容](https://mmbiz.qpic.cn/sz_mmbiz_jpg/TKUQcaFRv0EXljWzmpMV93XnFSnvOBXdDsWzib37PWdAicjFWHibUFRwtnib9aYvb0ibtY9sdSKRqJcpsn1e4N7acyDO0lNxaB5oIE2BDSdsnODs/640?wx_fmt=jpeg&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=0)
...
2. Prometheus 采集这些指标，例如 `vllm:num_requests_running` 和 `vllm:num_requests_waiting` 。
...
- `minReplicas` 、 `maxReplicas` ：限制最小、最大副本数
...
```
![KServe 模型服务自动扩缩容时间线](https://mmbiz.qpic.cn/sz_mmbiz_jpg/TKUQcaFRv0ENbjbGGO7fvr9gxOhJYeBLb53lJ4kLT0F8YmO1ggO8KRns50Z4Uw0WUGIbn4IrMXyGB1xpsYhXMfd50rHqRb857ibkNg6cnITE/640?wx_fmt=jpeg&from=appmsg&watermark=1#imgIndex=2)
...
3. KEDA 查询 Prometheus，并通过 External Metrics API 将查询结果暴露为 HPA 可以使用的外部指标。
4. HPA 比较指标当前值与目标值，计算期望副本数并调整 Predictor Deployment。
![KServe 集成 KEDA 的自动扩缩容链路](https://mmbiz.qpic.cn/mmbiz_jpg/TKUQcaFRv0GmgXPL8CDbsHaxHxPYx3vXQ9vQqZtIVc7s7DrWgZhXojLkmiaqtAysem3uEUlftgj3qkMicpL7u3lEFsUQibzmcgF5QI5NdRz6yE/640?wx_fmt=jpeg&from=appmsg&watermark=1#imgIndex=3)

KServe 集成 KEDA 的自动扩缩容链路

![图片](https://mmbiz.qpic.cn/sz_mmbiz_gif/TKUQcaFRv0EjHzZdq91G0BW8cHdJXhfmMILSmkqvjKCIcGD2LR2Xm9MRPLj54TKicWalZicC7cxMDPeU3XfOuAnIGDia47Nbb3qmKJiav5VsT2Y/640?wx_fmt=gif&from=appmsg#imgIndex=4)

往期回顾

[KServe + HAMi：一张 GPU 如何运行多个推理服务](https://mp.weixin.qq.com/s?__biz=Mzk0NzE5OTQyOQ==&mid=2247489955&idx=1&sn=b58a67e83819c63eb77436d22ade9e15&scene=21#wechat_redirect)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_gif/TKUQcaFRv0EjHzZdq91G0BW8cHdJXhfmMILSmkqvjKCIcGD2LR2Xm9MRPLj54TKicWalZicC7cxMDPeU3XfOuAnIGDia47Nbb3qmKJiav5VsT2Y/640?wx_fmt=gif&from=appmsg#imgIndex=5)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_gif/TKUQcaFRv0EjHzZdq91G0BW8cHdJXhfmMILSmkqvjKCIcGD2LR2Xm9MRPLj54TKicWalZicC7cxMDPeU3XfOuAnIGDia47Nbb3qmKJiav5VsT2Y/640?wx_fmt=gif&from=appmsg#imgIndex=6)
