...
Service 的主要作用，就是作为 Pod 的代理入口，代替 Pod 对外暴露一个固定的网络地址。
...
# controller
	...
	• pod有两种：1 自主式pod是直接创建出来的，删了就没了；2 由控制器创建出来的，删了还能自动重建。
...
deployment：controller中最常用的控制器

- 可以部署**无状态**应用，通过管理replicaset来管理pod
- 部署滚动升级，支持发布的停止和继续。