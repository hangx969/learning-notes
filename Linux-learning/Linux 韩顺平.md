...
- 创建一个新用户，不要用Root直接登陆
  ...
  想要用root用户登录的话，需要在登录界面点 not listed
...
- 具体目录结构：
  ...
  - /etc 系统管理所需要的配置文件和子目录  configuration
  
  - /opt 安装包 约定俗成 放在这里
  
  - **/usr** 用户安装的应用程序和文件  **/usr/local** 软件安装在这里
...
- 利用事先建立的查找数据库，快速定位文件；保证精确度，定时更新索引数据库
- 先 updatedb
- 再 locate xxx.txt
...
- sticky
  - 针对目录而言，如果设置了粘滞位，在该目录下的文件，只能是**root**和**创建者**可以删改，别的用户不能动别人的
...
- crontab 
  ...
  - 设置个人任务到调度文件：
    ...
    */1 * * * * ls -l etc/ > /tmp/to.txt  (每小时的每分钟执行)
    ...
    ![image-20231030200257984](https://raw.githubusercontent.com/hangx969/upload-images-md/main/202310302002081.png)
...
- 步骤3：格式化sdb1（指定文件格式）mkfs -t ext4 /dev/sdb1
...
- df -h ：查询磁盘占用情况
  ...
  - tmpfs是基于内存的文件系统, devtmpfs是系统内核启动的时候创建的文件系统，管理所有设备
...
- df （disk free）和 du (disk usage)的区别
  ...
  - 不同点：
    ...
    - 计算结果差异
      ...
      df可以获取已删除的文件，由于df是通过文件系统获取空间大小，当删除一个文件的嘶吼，他不是立即消失的，而是确认没有进程使用它才完全删除。

      而du只能看到当前存在的未删除的文件。

  > 通过df命令显示的磁盘使用情况是文件系统的使用情况，而du命令计算的是文件和目录的实际占用空间。
  >
  > df和du命令计算目录使用量的方法不同，df命令查看文件系统的使用情况时，该目录的空间占用会被计算在内，从而导致文件系统的使用情况显得比较高；而使用du命令计算该目录的使用情况时，则只会计算该目录下的文件大小总和，不会考虑文件系统的总容量和可用容量等信息，因此可能得出较低的结果。
  >
  > 如您需要扩充根目录，您可以确认下根目录挂载的分区：
  >
  > ```bash
  > lsblk -o NAME,KNAME,PATH,FSAVAIL,FSSIZE,FSTYPE,FSUSED,FSUSE%,MOUNTPOINT
  > ```
  >
  > ![image-20231030201222959](https://raw.githubusercontent.com/hangx969/upload-images-md/main/202310302012034.png)
...
6. lsblk -f: 列出所有块设备以及挂载信息，文件系统类型

7. fdisk -l: 查看所有硬盘分区情况

8. free -h: 看内存和虚拟内存（交换分区）
...
- top -d 秒数   （每隔5秒，默认是3 s）
  ...
  - sy：系统空间（内核程序）的CPU使用率

  - id：空闲CPU（99.8 id，表示99.8%的CPU未使用，反之使用率为0.2%）

  - 注意：top的%CPU显示的是CPU*核数，（一共4 core的CPU，%CPU到400%才算用满）:[Linux cpu为什么会超过100% ？ - Wolf_Stark - 博客园 (cnblogs.com)](https://www.cnblogs.com/wolfstark/p/16450131.html)

    ![image-20231030200827931](https://raw.githubusercontent.com/hangx969/upload-images-md/main/202310302008019.png)
...
1. 以RPM方式安装软件的系统-Redhat、Fedora、SuSe等
2. 以dpkg方式安装软件的系统-Ubuntu、Debian等
...
- 系统信息文件，记录系统事件，记录Linux各种运行信息，用于trouble-shooting
...
- 分类：
  ...
  - 内核和系统日志
    - 一般交给rsyslog统一管理，配置文件是 /etc/rsyslog.conf 规定了各种日志记录到什么位置
    - 大部分程序会把日志交给rsyslog统一管理
  - 用户日志
    - 记录用户登录退出的信息
  - 程序日志
    - 某些程序会自己管理日志
...
- 日志轮替
  - 将最近的日志留下来，时间久远的删除，节省空间；Logrotate不是系统守护进程，而是通过crond 每天执行
...
![image-20231030201329613](https://raw.githubusercontent.com/hangx969/upload-images-md/main/202310302013908.png)

![image-20231030201339581](https://raw.githubusercontent.com/hangx969/upload-images-md/main/202310302013677.png)
...
- 这一步的流程：显示加载两个镜像，再加载MOD模块文件，把grub2程序加载执行，接着解析配置文件/boot/grub/grub.cfg，根据配置文件加载内核镜像到内存，之后构建虚拟根文件系统，最后转到内核。
...
[How to: Linux delete or remove kernel - nixCraft (cyberciti.biz)](https://www.cyberciti.biz/faq/debian-redhat-linux-delete-kernel-command/)

- 按照这个文档可以安装一个最新的稳定内核版本：

  [教你三步在CentOS 7 中安装或升级最新的内核 | 《Linux就该这么学》 (linuxprobe.com)](https://www.linuxprobe.com/update-kernel-centos7.html)

- 安装完，重启之后，kernel版本会变成最新安装的。

- 从配置文件中修改启动默认的内核版本：

  - #查看开机选单中有几个内核，从此处选择合适的老版本

  ```bash
  cat /boot/grub2/grub.cfg | grep menuentry | grep -v grep
  ```

安装了新的内核之后，开机选单中会出现两个内核。在/etc/default/grub 中修改开机选单的默认选项

![image-20231030201646389](https://raw.githubusercontent.com/hangx969/upload-images-md/main/202310302016465.png)
