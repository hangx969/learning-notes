...
## 极端错误处理
panic: 可以在异常的时候让程序终止执行，退出程序。
- 只有在遇到不可恢复的致命错误时才用 ==panic/recover==，比如程序内部不一致、严重bug等。
- 普通业务逻辑错误（如除零、文件不存在）不影响程序其他部分，应通过 error 返回。
- panic不能滥用，不能到处用panic
...
>    3. recover会阻止panic一层一层传播导致程序崩溃。recover会捕获到panic的报错，尝试恢复程序，不让其终止。recover必须在defer中执行。
...
func test() {
	...
	//defer注册一个延迟调用的匿名函数。
	defer func() {
		// recover()内置函数，没panic返回nil；有panic返回panic值
