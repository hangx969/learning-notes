...
func getMinMax(i1, i2 int) (min, max int) {
	if i1 > i2 {
		max = i1
		min = i2
	} else {
...
defer可以在函数return之前或者程序panic之前执行，一般用于资源回收和数据返回，也可以用于异常恢复。

好习惯是打开一个文件之后，马上接一个defer去关闭这个文件。
...
>
> 在 golang 编程中的通常做法是：
> 1. 创建资源后，比如打开了文件，获取了数据库的链接，或者是锁资源，可以直接在后面执行 `defer file.Close()` `defer connect.Close()`
...
**相关笔记：** [[go-基础-01-环境配置-基础]] | [[go-基础-03-变量-数据类型-运算]] | [[go-基础-04-分支-循环]] | [[go-基础-06-错误处理]] | [[go-面向对象]]
