...
1. url: (必填) 目标资源的 URL 地址,表示你希望更新的服务器资源的路径。
2. data: (可选) 发送到服务器的更新数据,通常是**字典格式**的内容。
3. **kwargs: (可选) 其他可选参数,类似于请求头(headers)、超时时间(timeout)、认证信息(auth)等。
...
==delete()== 方法用于发送 HTTP DELETE 请求,以删除服务器上的特定资源。
...
==head()== 方法用于发送 HTTP HEAD 请求,功能类似于 GET 请求,但**只请求响应头而不下载响应体**。该方法通常用于获取关于特定资源的信息,例如检查资源是否存在、获取内容类型、获取缓存信息等,而不需要下载整个内容。
...
> [!tip] 效率优势
> 由于不下载响应体,HEAD 请求比 GET 请求更高效,适用于需要快速检查资源信息的场景。
...
==options()== 方法用于发送 HTTP OPTIONS 请求,目的是查看服务器支持的请求方法。
...
使用 ==timeout== 参数可以设置请求的最大等待时间,以防止请求无限期挂起。超过此时间就会抛出Timeout异常。
...
- 如果你希望手动处理重定向,可以通过设置 `allow_redirects` 参数为 ==False== 来禁用自动重定向。
