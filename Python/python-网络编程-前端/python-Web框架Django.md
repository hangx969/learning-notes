...
> 功能:通过这个脚本,你可以运行常用的 Django 命令,如启动开发服务器 (==runserver==)、数据库迁移 (==migrate==)、创建管理员账号(==createsuperuser==) 等。
> 示例:`python manage.py runserver`

### 内层 myproject/(项目应用目录)

这个文件夹是项目的实际应用目录,它包含与 Django 框架相关的核心配置文件和代码(如settings.py、urls.py、asgi.py、wsgi.py、\_\_init\_\_.py)。目录结构示例:
...
>
> 1. 第一个参数 `''` 是空字符串,它表示网站的根 URL。
> 2. 第二个参数 `home` 是你之前定义的视图函数。
> 3. 第三个参数 `name='home'` 给这个路由起了一个名字。通过给路由命名,可以在 Django 项目的其他地方方便地引用这个 URL。
...
> [!info] ORM简介
> ==ORM==是对象关系映射模型,是 Django 框架中用于与数据库进行交互的模型。它允许使用 Python 类来定义数据库表,并通过 Python 代码执行 SQL 操作,如查询、插入、更新和删除数据。
>
...
> [!important] 迁移机制
> 在 Django 中,迁移(migrations)是管理数据库模式变化的一种机制。每当你对模型类进行更改时,Django 会生成迁移文件,以便将这些更改应用到数据库中。
...
==makemigrations== 的作用:创建迁移文件。当你运行 `python manage.py makemigrations` 时,Django 会检查你在模型类中所做的更改,并生成相应的迁移文件。这些文件包含了需要在数据库中执行的操作,比如创建表、添加字段等。
...
> [!note] Django模板语法
> - `{% for article in articles %}`: 这是 Django 模板语言,表示一个循环。它会遍历 articles 变量中的每一个元素。
> - `{{ article.title }}` 是 Django 模板中的变量表达式,用来获取 article 对象的 title 字段。
...
> [!info] 内置用户模型
> 在 Django 中,==User== 模型(从 `django.contrib.auth.models` 导入)提供了内置的用户管理功能。
...
在 myapp/urls.py 文件中,为 register 视图创建路由:
...
在 templates 文件夹中创建 register.html:
...
> [!note] 认证函数
> 使用 Django 内置的 ==authenticate== 和 ==login== 函数来处理用户认证和登录。
