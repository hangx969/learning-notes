...
> [!summary] 双向通信流程
> 1. **服务端**:等待客户端连接。连接成功后,服务端向客户端发送"python"。然后等待从客户端接收数据。当接收到客户端发送的数据"Hello from client"时,打印输出。
> 2. **客户端**:服务端向客户端发送"python",客户端接收到并打印。客户端向服务端发送"Hello from client",服务端接收并打印。
>
> 这样双方完成一次双向数据传输。
...
> [!summary] 需求
> 构建一个简单的聊天服务器,允许多个客户端连接并交换信息。使用 ==threading== 实现多线程处理。
