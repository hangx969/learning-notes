---
title: Web开发基础 - HTML/CSS/JavaScript
tags:
  - python/web
  - python/frontend
  - web/html
...
| ------------------ | ------------------------------------------------------------ |
| id="section1"      | 创建锚点                                                     |
| a href="#section1" | 链接的`href`属性指向了这些锚点的名称，当用户点击链接时，浏览器会自动滚动到相应的位置。 |
...
CSS内部样式表是一种将CSS规则集直接写在HTML文件内部的方法，通过在HTML文件的\<style>标签内部编写CSS代码。
...
使用场景：
1.	特定页面样式：对于只需要在特定页面或特定部分应用样式的情况。
2.	临时样式调整：在开发阶段或调试过程中临时调整样式。
3.	样式独立性：不希望其他页面或元素受到影响。

> [!note] 样式表对比
> 在实际应用中，==外部样式表==的使用频率更高一些，特别是对于大型项目和多页面网站。内部样式表通常用于一些特定的场景，使用频率相对较低。

---

#### 内联样式表

CSS内联样式表是一种直接将 CSS 样式规则写在 HTML 元素的 style 属性中的方法。
...
内联样式表的使用相对较少，通常只在个性化样式、邮件和简单页面、单个样式元素等特定场景下使用。
...
> [!info] 边框样式类型
> ==solid==：实线边框 | ==dashed==：虚线边框 | ==dotted==：点线边框 | ==double==：双线边框 | ==groove==：3D凹陷边框 | ==ridge==：3D凸起边框 | ==inset==：3D内嵌边框 | ==outset==：3D外嵌边框 | ==none==：无边框
...
> [!info] 边距概念
> 1. ==内边距(padding)==：控制元素内部内容与其边框之间的距离。
> 2. ==外边距(margin)==：控制元素与其相邻元素之间的距离。
...
CSS 的 ==display== 属性定义了一个元素应该如何显示在页面中。通过设置不同的 display 值，可以使元素以块级、内联、弹性盒子等形式进行布局。

> [!note] 块级元素 vs 内联元素
> **块级元素**：独占一行，可以设置宽度、高度、外边距和内边距。常见有 `<div>`、`<p>`、`<h1>` 等。
>
> **内联元素**：在同一行内显示，不会独占一行，不可以设置宽度和高度。常见有 `<span>`、`<a>`、`<strong>` 等。
...
==display: flex== 表示将一个容器元素变成一个 "Flex Container"（弹性容器），容器内的子元素将会按照一定的规则进行布局。

> [!tip] Flex的常用属性
> 1.	`flex-direction`: 定义主轴的方向（row/column/row-reverse/column-reverse）
> 2.	`justify-content`: 定义项目在主轴上的对齐方式
> 3.	`align-items`: 定义项目在交叉轴上的对齐方式
> 4.	`flex-grow`: 定义弹性元素的放大比例
> 5.	`flex-shrink`: 定义弹性元素的缩小比例
> 6.	`flex-basis`: 定义弹性元素在主轴上的初始大小
> 7.	`flex-wrap`: 是否换行（nowrap为默认不换行）
...
CSS RWD 的主要特点：
1.	==弹性网格布局==：使用相对单位（如百分比）来定义网格和布局。
2.	==媒体查询==：使用媒体查询可以根据设备特性应用不同的CSS样式。
3.	==弹性图片==：通过设置图片的最大宽度为100%，确保图片在不同设备上不会超出其容器。
4.	==视口设置==：使用视口标签可以控制页面在移动设备上的显示方式。
...
> [!tip] 常用CSS属性
> - `max-width: 100%` + `height: auto`: 确保图片在其容器内按比例缩放
> - `width: 100%` + `height: auto`: 让图片填满其容器的宽度
...
> [!note] 视口标签说明
> 这段代码告诉浏览器：让网页的宽度和设备的宽度一样，并且初始缩放比例应该为==1.0==。
> 这确保网页在移动设备上显示时，大小合适，并且不会自动进行缩放。
...
1. ==for循环==：用于已知循环次数的情况
2. ==while循环==：用于未知循环次数的情况
3. ==if..else==：根据条件执行不同的代码块
4. ==switch==：根据表达式的值执行不同的代码块
