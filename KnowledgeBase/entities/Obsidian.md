...
- [[AI/Obsidian/obsidian-claude-搭建个人知识库]]：3 种集成工具的完整安装配置指南
- [[AI/Obsidian/obsidian-claude-code-AI知识管家]]：为什么 Obsidian+Claude Code 是最优解、30 分钟四步上手、Karpathy Wiki 模式、自动 backlinks
...
> 来源：[告别公众号排版烦恼：Obsidian一键发布插件使用指南](https://mp.weixin.qq.com/s/8_yCe-kILmRJsCpbb_hArw)
> 插件名：Wechat Public Platform | 作者：ai-chen2050

在 Obsidian 中写完文章后一键发布到微信公众号，省去手动复制、排版、上传图片的流程。
...
| 发布文章 | 一键发布到微信公众号 |
| 群发消息 | 推送给所有粉丝（需认证） |
| 下载素材 | 从公众号后台下载已发布的文章/图片/视频/音频 |
| 发布到百家号 | 多平台分发 |
| 数据统计 | 用户数据和文章概况面板 |

**安装**：Obsidian 插件市场搜索 "Wechat Public Platform"

**配置**（一次性）：
1. 从微信公众平台获取 **AppID** + **AppSecret**（设置与开发 → 基本配置）
2. 在插件设置中填入
3. 将本机外网 IP 添加到微信公众平台的 **IP 白名单**

**使用**：在文章 frontmatter 中设置发布参数：

```yaml
---
author: 名字
banner_path: "cover.jpg"       # 本地封面图
banner: "https://..."          # 网络封面图（备选）
thumb_media_id: "awM_2hM..."   # 微信素材库封面（优先级最高）
open_comment: 0                # 0=关闭评论，1=开启
digest: "文章摘要"
---
```

然后执行命令「微信公众号发布文章」或「微信公众平台添加草稿」即可。

> [!note] 权限说明
> 基础发布功能个人订阅号即可使用。群发消息需要已认证的公众号。
