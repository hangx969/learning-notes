...
### 安全加固
- **Capabilities 最小化**：`--cap-drop ALL` 后按需 `--cap-add`，将默认 14 项能力缩减到最小集合
- **防提权**：`--security-opt=no-new-privileges:true` 阻止 suid/capabilities 提权，应始终启用
- **User Namespace 重映射**：`"userns-remap": "default"` 将容器内 root 映射为宿主机非特权用户
