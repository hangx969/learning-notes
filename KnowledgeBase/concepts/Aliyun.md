---
title: Aliyun
tags:
  - knowledgebase/concept
date: 2026-04-16
---

# Aliyun

## 定义
阿里云（Alibaba Cloud）是阿里巴巴集团旗下的公有云平台，提供计算（ECS）、网络（VPC、SLB、WAF、DDoS）、存储（OSS）、数据库（RDS）等全栈云服务。本仓库以阿里云 ACP 认证备考和实战运维为主线进行知识整理。

## 在本仓库中的位置
主要集中在 `Aliyun/` 目录下，按计算、网络、存储、数据库、资源管理等子目录组织，共 19 篇文章。

## 相关文章
- [[Aliyun/ACP考试]]
- [[Aliyun/计算/ECS]]
- [[Aliyun/计算/弹性伸缩ESS]]
- [[Aliyun/计算/云盘-快照-镜像]]
- [[Aliyun/计算/主机迁移工具SMC]]
- [[Aliyun/网络/VPC]]
- [[Aliyun/网络/负载均衡SLB]]
- [[Aliyun/网络/WAF]]
- [[Aliyun/网络/DDoS高防]]
- [[Aliyun/网络/DNS-CDN-SSL]]
- [[Aliyun/网络/CEN-TR]]
- [[Aliyun/网络/网关-VPN-专线]]
- [[Aliyun/网络/云安全中心-云防火墙]]
- [[Aliyun/存储/对象存储OSS]]
- [[Aliyun/存储/数据湖-HDFS-POSIX]]
- [[Aliyun/存储/跨域共享CORS]]
- [[Aliyun/数据库/关系型数据库RDS]]
- [[Aliyun/数据库/数据传输服务DTS]]
- [[Aliyun/资源管理/Landing Zone]]

## 关联概念
- [[KnowledgeBase/concepts/Azure]]
- [[KnowledgeBase/concepts/Terraform]]
- [[KnowledgeBase/concepts/自动化运维]]

## 可延展方向
- 阿里云容器服务 ACK 与 Kubernetes 实践
- 阿里云 Terraform Provider 资源编排
- 多云架构设计（Azure + Aliyun 混合云）
- 阿里云日志服务 SLS 与可观测性
- RAM 权限管理与安全合规
